...
public class Model
{
	...
	public void arrive(int common, double time)
	{
		...
		if (server_status[common] == 1)
		{
			...
			time_arrival[common][num_in_queue[common]] = time;
		}
	...
	public void depart(int common)
	{
		...
		++num_custs_delayed[common];
                if(common==1)
                {
                    if(r.nextDouble() > 0.0 && r.nextDouble()<=0.4)
                    {
			arrive(2,sim_time);
			update_time_avg_stats(2);
                    }
                    else if(r.nextDouble() > 0.4 && r.nextDouble() <= 0.7)
                    {
			arrive(3,sim_time);
	...
	{
		return (double) (down+((up-down)*random.nextDouble()));
	}
